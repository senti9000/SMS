...
class StudentProfileForm(forms.ModelForm):
    course = forms.ModelChoiceField(queryset=Course.objects.all(), required=True, empty_label="Choose a course")
    ...
    class Meta:
        ...
        fields = [
            ...
            'place_of_birth', 'date_of_birth', 'gender', 'address', 'phone',
            'year_level', 'course', 'department_name', 'student_type',
            ...
            'junior_high_name_year_graduated', 'senior_high_name_year_graduated', 'f137', 'psa_photocopy',
            'shs_diploma_photocopy', 'good_moral', 'honorable_dismissal',
            'original_tor', 'pictures', 'profile_pic'
        ]
        widgets = {
            ...
            'shs_diploma_photocopy': FileInput(attrs={'id': 'id_shs_diploma_photocopy'}),
    ...
    def __init__(self, *args, user=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.user = user
        # Disable student_number field if instance exists (updating)
        if self.instance and self.instance.pk:
            if 'student_number' in self.fields:
                self.fields['student_number'].disabled = True
            # Disable student_status only if user is not superuser
            if 'student_status' in self.fields and not (self.user and self.user.is_superuser):
...
class ChangeStudentSubjectForm(forms.Form):
    ...
    semester = forms.ChoiceField(
        choices=[('', 'Choose semester')] + Student.SEMESTER_CHOICES,
        required=True,
        label="Semester"
    )
