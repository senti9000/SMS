...
def drop_student_subject_ajax(request):
    try:
        ...
        # Update subject status to 'Drop'
        subject.status = 'Drop'
        subject.save()

        # Optionally, mark related grades as inactive
        grades = Grade.objects.filter(student_id=student_id, subject_id=subject_id, is_active=True)
        for grade in grades:
            grade.is_active = False
            grade.save()

        return JsonResponse({'success': True, 'message': 'Subject status updated to Drop and grades marked inactive.'})
...
@student_required  # Ensure only student users can access this view
def student_dashboard(request):
    ...
    return render(request, 'SMS.html', {'user': request.user, 'student': student, 'departments': departments})
...
def subject_list(request):
    # Temporarily disable search filter and pagination for debugging
    ...
    # Use all subjects without filtering or pagination
    grouped_by_year = {}
    for subject in subjects:
        year = subject.year_level if subject.year_level is not None else "Unassigned Year"
        course_name = subject.course_id.name if subject.course_id and subject.course_id.name else "Unassigned Courses"
        if year not in grouped_by_year:
            grouped_by_year[year] = {}
        if course_name not in grouped_by_year[year]:
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            if existing_student_by_name and (not existing_student_by_name.user or is_account_incomplete(existing_student_by_name.user, existing_student_by_name)):
                ...
                if profile_form.is_valid():
                    ...
                    # Preserve existing student_number and student_status to prevent deletion
                    student.student_number = existing_student_by_name.student_number
                    student.student_status = existing_student_by_name.student_status
                    student.user = user
                    student.email = user.email
                    student.username = user.username
                    student.save()
...
def edit_grade(request, student_id, subject_id):
    ...
    # Get or create the Grade instance for the student, subject, semester, and academic_year
    ...
    # Prepare grades dictionary for the student
    ...
    def update_subject_status_based_on_grade(grade_obj):
        # Update subject status to 'Done' only if grade_value is a valid completed grade
    ...
    if request.method == 'POST':
        try:
            ...
            if not created:
                ...
                grade.save()
                logging.info(f"edit_grade_ajax: Updated existing grade for student_id={student_id}, subject_id={subject_id}")
            else:
                logging.info(f"edit_grade_ajax: Created new grade for student_id={student_id}, subject_id={subject_id}")

            # Optionally update subject status if grade_value is set
            if grade_value is not None and grade_value != '' and grade_value != '-':
...
def change_subject_ajax(request):
    if request.method == 'POST' and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            # Add new subject's grade with default or empty values
            from .models import Grade
            Grade.objects.get_or_create(
                student=student,
                subject=new_subject,
...
def student_record(request, pk):
    ...
    try:
        ...
        # Get all subjects for the student's course without filtering by year_level or semester
        ...
        # Ensure all grades have academic_year set to student's academic_year if missing
        for grade in active_grades_qs:
        ...
        logging.info(f"student_record view: student_id={student.id}, total subjects={len(all_subjects_qs)}")

        subjects = Subject.objects.filter(department_name=student.department_name).order_by('year_level', 'semester_offered', 'subject_name')
...
@login_required
def SMSstaffvstu(request, subject_id):
    search_query = request.GET.get('search', '')
    from django.db.models import Q
    ...
    if subject:
        ...
        students = Student.objects.filter(course__department_name__name="College of Computer Studies")
    ...
    # Convert defaultdict to regular dict for template context
    grouped_students = dict(grouped_students)
    return render(request, 'staffpage/SMS(staffvstu).html', {'grouped_students': grouped_students, 'search_query': search_query, 'subject_id': subject_id})
...
def SMSstaffvstuA(request, subject_id):
    ...
    # Convert defaultdict to regular dict for template context
    grouped_students = dict(grouped_students)
    return render(request, 'staffpage/SMS(staffvstuA).html', {'grouped_students': grouped_students, 'search_query': search_query})
